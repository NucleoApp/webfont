...
exports.default = function (initialOptions) {
    ...
    return buildConfig({
        ...
        .then(function (svgFont) {
            ...
            result.svg = svgFont;
            result.ttf = Buffer.from((0, _svg2ttf2.default)(result.svg.toString(), {
            ...
            return (0, _globby2.default)(buildInTemplateDirectory + '/**/*').then(function (buildInTemplates) {
                ...
                if (Array.isArray(options.template)) {
                    ...
                    templatesArray = templatesArray.map(function (item) {
                        return buildInTemplateDirectory + '/template.' + item + '.njk';
                    });
                } else {
                    var _templateFilePath = options.template;
                    if (supportedExtensions.indexOf('.' + options.template) !== -1) {
                        ...
                        _templateFilePath = buildInTemplateDirectory + '/template.' + options.template + '.njk';
                    } else {
                        _templateFilePath = _path2.default.resolve(_templateFilePath);
                    }
                }
                ...
                if (templatesArray) {
                    templatesArray.forEach(function (item, index) {
                        var title = options.template[index];
                        result[title] = _nunjucks2.default.render(item, nunjucksOptions);
                    });
                } else {
